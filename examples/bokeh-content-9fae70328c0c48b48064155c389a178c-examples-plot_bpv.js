(function() {
  const fn = function() {
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
    
    const element = document.getElementById("4b7a32f2-71cb-4ebb-8221-64aaa95d1f67");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '4b7a32f2-71cb-4ebb-8221-64aaa95d1f67' but no matching script tag was found.")
        }
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.3.min.js"];
      const css_urls = [];
    
      const inline_js = [    function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                  const docs_json = '{"323d8680-d4c9-48f3-be39-9b2d2bee67a5":{"defs":[],"roots":{"references":[{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"18428","type":"Title"},{"attributes":{},"id":"18374","type":"DataRange1d"},{"attributes":{},"id":"18383","type":"BasicTicker"},{"attributes":{"data":{},"selected":{"id":"18442"},"selection_policy":{"id":"18441"}},"id":"18422","type":"ColumnDataSource"},{"attributes":{},"id":"18440","type":"Selection"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"18398","type":"BoxAnnotation"},{"attributes":{"overlay":{"id":"18398"}},"id":"18392","type":"BoxZoomTool"},{"attributes":{},"id":"18393","type":"WheelZoomTool"},{"attributes":{"line_alpha":0.2,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"18419","type":"Line"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18423","type":"Line"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"18438"},"selection_policy":{"id":"18437"}},"id":"18409","type":"ColumnDataSource"},{"attributes":{"coordinates":null,"formatter":{"id":"18435"},"group":null,"major_label_policy":{"id":"18436"},"ticker":{"id":"18383"}},"id":"18382","type":"LinearAxis"},{"attributes":{},"id":"18390","type":"ResetTool"},{"attributes":{},"id":"18396","type":"SaveTool"},{"attributes":{},"id":"18441","type":"UnionRenderers"},{"attributes":{},"id":"18436","type":"AllLabels"},{"attributes":{},"id":"18395","type":"UndoTool"},{"attributes":{},"id":"18442","type":"Selection"},{"attributes":{},"id":"18438","type":"Selection"},{"attributes":{"callback":null},"id":"18397","type":"HoverTool"},{"attributes":{"children":[[{"id":"18373"},0,0]]},"id":"18446","type":"GridBox"},{"attributes":{},"id":"18437","type":"UnionRenderers"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"18440"},"selection_policy":{"id":"18439"}},"id":"18416","type":"ColumnDataSource"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"18399","type":"PolyAnnotation"},{"attributes":{},"id":"18433","type":"AllLabels"},{"attributes":{},"id":"18391","type":"PanTool"},{"attributes":{},"id":"18435","type":"BasicTickFormatter"},{"attributes":{"source":{"id":"18422"}},"id":"18427","type":"CDSView"},{"attributes":{"toolbars":[{"id":"18400"}],"tools":[{"id":"18390"},{"id":"18391"},{"id":"18392"},{"id":"18393"},{"id":"18394"},{"id":"18395"},{"id":"18396"},{"id":"18397"}]},"id":"18447","type":"ProxyToolbar"},{"attributes":{},"id":"18378","type":"LinearScale"},{"attributes":{"children":[{"id":"18448"},{"id":"18446"}]},"id":"18449","type":"Column"},{"attributes":{},"id":"18432","type":"BasicTickFormatter"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18411","type":"Line"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18410","type":"Line"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18425","type":"Line"},{"attributes":{},"id":"18380","type":"LinearScale"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"18417","type":"Line"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"18412","type":"Line"},{"attributes":{"coordinates":null,"data_source":{"id":"18409"},"glyph":{"id":"18410"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18412"},"nonselection_glyph":{"id":"18411"},"view":{"id":"18414"}},"id":"18413","type":"GlyphRenderer"},{"attributes":{},"id":"18376","type":"DataRange1d"},{"attributes":{"coordinates":null,"data_source":{"id":"18422"},"glyph":{"id":"18423"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18425"},"nonselection_glyph":{"id":"18424"},"view":{"id":"18427"}},"id":"18426","type":"GlyphRenderer"},{"attributes":{},"id":"18387","type":"BasicTicker"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"18418","type":"Line"},{"attributes":{"below":[{"id":"18382"}],"center":[{"id":"18385"},{"id":"18389"},{"id":"18415"}],"height":500,"left":[{"id":"18386"}],"output_backend":"webgl","renderers":[{"id":"18413"},{"id":"18420"},{"id":"18426"}],"title":{"id":"18428"},"toolbar":{"id":"18400"},"toolbar_location":null,"width":500,"x_range":{"id":"18374"},"x_scale":{"id":"18378"},"y_range":{"id":"18376"},"y_scale":{"id":"18380"}},"id":"18373","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"18439","type":"UnionRenderers"},{"attributes":{"axis":{"id":"18386"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"18389","type":"Grid"},{"attributes":{"coordinates":null,"formatter":{"id":"18432"},"group":null,"major_label_policy":{"id":"18433"},"ticker":{"id":"18387"}},"id":"18386","type":"LinearAxis"},{"attributes":{"coordinates":null,"data_source":{"id":"18416"},"glyph":{"id":"18417"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"18419"},"nonselection_glyph":{"id":"18418"},"view":{"id":"18421"}},"id":"18420","type":"GlyphRenderer"},{"attributes":{"overlay":{"id":"18399"}},"id":"18394","type":"LassoSelectTool"},{"attributes":{"axis":{"id":"18382"},"coordinates":null,"group":null,"ticker":null},"id":"18385","type":"Grid"},{"attributes":{"source":{"id":"18409"}},"id":"18414","type":"CDSView"},{"attributes":{"tools":[{"id":"18390"},{"id":"18391"},{"id":"18392"},{"id":"18393"},{"id":"18394"},{"id":"18395"},{"id":"18396"},{"id":"18397"}]},"id":"18400","type":"Toolbar"},{"attributes":{"bottom":1.459677659268402,"coordinates":null,"fill_alpha":0.1,"fill_color":"#2a2eec","group":null,"top":0.6850827603275131},"id":"18415","type":"BoxAnnotation"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"18424","type":"Line"},{"attributes":{"toolbar":{"id":"18447"},"toolbar_location":"above"},"id":"18448","type":"ToolbarBox"},{"attributes":{"source":{"id":"18416"}},"id":"18421","type":"CDSView"}],"root_ids":["18449"]},"title":"Bokeh Application","version":"2.4.3"}}';
                  const render_items = [{"docid":"323d8680-d4c9-48f3-be39-9b2d2bee67a5","root_ids":["18449"],"roots":{"18449":"4b7a32f2-71cb-4ebb-8221-64aaa95d1f67"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
    function(Bokeh) {
        }
      ];
    
      function run_inline_js() {
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();